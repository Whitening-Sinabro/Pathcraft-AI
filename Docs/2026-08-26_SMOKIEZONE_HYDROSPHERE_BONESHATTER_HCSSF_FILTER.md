...
### 글자색 채널의 구현 규칙 (2026-08-28 수정)

인게임에서 Unique Crude Bow 라벨이 갈색 배경에 갈색 글자로 찍혀 읽히지 않았다. 원인은 Continue 캐스케이드다. 가져온 Death Oath 고유 티어 규칙은 일반 고유 갈색 `175 96 37`을 **배경**으로 칠하고 글자는 검정/흰색으로 두는데, 그 뒤에 놓인 "글자색만 갈색으로 덮는 가드"가 글자를 배경과 같은 색으로 만들었다. 장비 4,640 조합 시뮬레이션에서 같은 구조의 충돌이 587건(Unique·Magic·Rare 전부)이었고, 베이스 Luminary 필터는 0건이었다.

최종 규칙은 다음 셋이다.

- 장식 규칙 뒤의 **글자색 전용 가드는 젬(Cyan)만** 남긴다. 희귀도 글자색 기본값은 장식 규칙 **앞**에 둔다 — Normal/Magic/Rare는 감사 기준선, 일반 Unique는 Death Oath의 `Rarity Unique` 마젠타 리셋 규칙을 `SMOKIEZONE - ORDINARY UNIQUE DEFAULT`(갈색 글자 / 어두운 배경)로 치환해서 얻는다. 무조건 마젠타 리셋 규칙은 제거한다.
- 이 필터가 생성하는 블록이 **배경을 칠하면 글자색도 반드시 자기 것으로 가진다.** 여러 희귀도를 묶던 제작 베이스·플라스크 블록은 희귀도별로 나뉜다(`- NORMAL / - MAGIC / - RARE`). 희귀도 조건이 없는 블록(예: `of Infamy` 투구)은 배경 명도에 맞는 중립 글자색을 받는다.
- 생성기의 검증 단계가 `scripts/filter_cascade.py`로 캐스케이드를 시뮬레이션해 장비 아키타입 전부(BaseItemTypes 기준 4,000여 조합)의 글자/배경 대비를 재고, 1.6 미만이 하나라도 있으면 빌드를 실패시킨다.

시뮬레이션은 조건 일부만 모델링하므로 실제 게임보다 **덜** 잡을 수 있다. 인게임에서 새로 보이는 안 읽히는 라벨은 다시 신고한다.
