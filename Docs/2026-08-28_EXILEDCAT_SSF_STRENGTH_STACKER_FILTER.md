...
검증 게이트(빌드 실패 조건): 3.29 GGPK `BaseItemTypes`/`ActiveSkills` 에 없는 이름, **빌더에 고정(pin)된 필수 타깃 id·고유 베이스 누락/변경**(`REQUIRED_TARGET_IDS` — 스펙 편집으로 타깃이 조용히 빠지는 것을 막는 스펙 밖 목록), Wrecker SSF Hide 블록 87개 변경, 생성 블록이 배경만 칠하고 글자색 미소유, 캐스케이드 대비 < 1.6(`scripts/filter_cascade.py`, 장비 아키타입 전수), 사운드 파일 부재, Hide 블록의 소리/빔/아이콘 누출.

적대검증(2026-08-28, fresh verifier) 반영: 시뮬레이터가 `Hide` + `Continue` 뒤 무매치를 Show 로 보고하던 결함 수정(마지막 매치 action 유지), `HasSearingExarchImplicit >= 1` 류 수치 조건을 boolean 으로 오파싱하던 결함 수정, OPTIONAL 고유 블록을 NeverSink T0/T1 앞으로 이동(Timeless Jewel 이 T1 스타일로 덮이던 dead code 해소), 카드 사다리 주석을 실제 버킷 수로 생성, 스펙 자기참조 검증을 pin 목록으로 보강.
...
| 선택 고유 베이스 (소리·빔 없음, 보라 별 아이콘) | Timeless Jewel — Part 2 예고 Lethal Pride, 다른 타임리스 주얼도 같이 표시. 크리에이터 선택 블록이 NeverSink T1 목록(Timeless Jewel 포함)보다 앞에 놓여 경제 티어 스타일로 덮이지 않는다 | 5.5 / 9.6 |
...
| 자원 | Cyaxan's Ducat · Crystallised Lifeforce 3종 · Rogue's Marker / Eldritch Ember·Ichor 8종 / Essence of Rage·Greed / Breachstone·Splinter·Wombgift / Blueprints(전부) · Contracts — important 그룹은 기본음 2/210, crafting 그룹(Essences·Contracts)은 기본음 2/170 + 짧은 빔 + 육각/십자 아이콘 | 8.1·8.2 / 4.2 임플리싯 / 5.1·5.3·9.5 / 5.2 |
