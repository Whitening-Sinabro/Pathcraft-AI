...
- [x] `delve_advisor.py` CLI 단독 실행 → JSON stdout 출력
- [x] POB URL 입력 → life/ES/저항 정상 추출
- [x] poe.ninja fossil 가격 fetch (실패 시 폴백 허용)
- [x] 깊이별 안전 범위 + 화석 추천 출력
- [x] WPF "⛏️ Delve" 탭에서 분석 버튼 동작
- [x] 네트워크 없어도 크래시 없음
