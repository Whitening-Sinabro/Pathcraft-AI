...
| 자료 기반 개인용 빌드 생성 제품으로 발전시킬 수 있는가? | **두 사례의 제작·검증 성과와 재사용 기반이 있다. 범용 제품으로의 확대·연결은 남아 있다.** | 코칭 JSON과 자료 검색·추천·규칙 검증, 단계별 플래너·전환 근거가 있다. 그러나 사용자 조건을 반영한 스킬·지원젬·패시브·장비·자원·성장 단계의 일관된 구성을 만들고 검증하는 연결을 따로 확인해야 한다. 자세한 생성 경로와 자산의 연결 정도는 [생성 준비도 보고서](build_generation_readiness.md)를 따른다. |
| 지금 개인용 생성은 어디까지 가능한가? | **임성빈·스카두쉬에서 자료 검증→조건·순서 결정→가이드/.build 제작 방식은 이미 실증됐다. 다른 빌드·사용자 조건으로의 반복 적용과 앱 자동화는 미완료다.** | [두 사례 검증 근거서](case_validation_evidence.md)에 실제 제작·정정 성과를 정리했다. 현재 앱에는 POE2의 PoB 없는 코칭 입력이 있으나, 두 사례의 제작 절차를 사용자 조건별 생성·검증·수정·저장으로 자동 연결한 범위는 미완료다. |
...
**두 사례의 성과를 명시적으로 정정한다.** 임성빈 화염파 젬링과 스카두쉬 토템·함성 워브링어는 자료 수집을 넘어 실제 구성·성장/전환 순서를 교차확인하고 오독과 충돌을 정정하여 개인용 가이드·플래너에 반영한 사례다. 앞선 자료/추천/변환 중심 요약은 이 성과를 충분히 반영하지 못했다. 사례 검증은 이미 존재하며, 다른 스트리머·패치로의 일반화와 사용자 조건별 앱 자동화가 미완료다. [두 사례의 정확한 대응·성과·한계](case_validation_evidence.md).
...
후속 사용자 보정에 따라 [두 사례 검증 근거서](case_validation_evidence.md)를 별도 동봉했다. 다섯 본 보고서 중 종합·생성 판정 단락만 보완했으며 직전 판과 manifest는 `before_case_validation_correction/`에 보존했다.
