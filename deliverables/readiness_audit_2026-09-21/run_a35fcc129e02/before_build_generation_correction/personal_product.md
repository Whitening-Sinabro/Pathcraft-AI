...
**개인용으로 쓸 수 있는 연구·가이드·게임 플래너 자산은 있다. 그러나 일반 사용자가 앱에서 `내 캐릭터 입력 → 목표 선택 → 차이 확인 → 다음 단계 실행 → 저장·재열기`를 완결하는 개인용 제품은 아직 입증되지 않았다.** 특히 최근 POE2 자산은 실체가 풍부하지만 앱의 주요 입력·데이터 경로와 분리돼 있다. POE1은 앱 안의 PoB 분석·리서치·가이드 표시 경로가 더 연결돼 있으며, POE2는 수동으로 자료를 고르고 외부 게임 플래너와 함께 사용하는 개인 연구 환경으로서 가치가 더 크다. 근거는 [POE1 선택 연결](D:/Pathcraft-AI/src/App.tsx:754), [POE2 구두 입력 분기](D:/Pathcraft-AI/src/App.tsx:933), [등록된 앱 명령](D:/Pathcraft-AI/src-tauri/src/lib.rs:1152), [HC Journey의 남은 UI/API](D:/Pathcraft-AI/python/hc_journey/README.md:130)다.

완성률·개발 기간·매출은 추정하지 않는다. 개인용 최소 완결판의 중심은 새 자료를 더 많이 모으는 것보다 **이미 있는 자료에 현재/목표 역할, 적용 패치, 모드, 관측 시점, 전환 조건을 붙여 한 흐름으로 연결하는 것**이다.
...
**현재 개인 자료 활용을 우선한다면 POE2 임성빈 젬링의 한 전환 구간을 첫 수직 흐름으로 권고한다.** 최근 실제 개인 플래너·원본 관측·전환 조건이 그곳에 모여 있고, 현재 상태 문서도 그 사용을 중심으로 움직인다. Skadoosh는 상태 문서에서 사용자가 버린 빌드로 적혀 있어 기본 개인 목표로 다시 채택할 이유가 없다. 반대로 기존 앱 연결을 먼저 완결하는 것이 우선이면 POE1 한 빌드가 더 연결된 출발점이다. 이는 제품 범위의 권고이며 POE1/POE2 전체 지원을 새로 약속하거나 사용자 목표를 대신 변경한 것이 아니다. [개인 우선 자료](D:/Pathcraft-AI/.claude/status/current.md:1), [Skadoosh 사용 맥락](D:/Pathcraft-AI/.claude/status/current.md:11), [POE1 기존 연결](D:/Pathcraft-AI/src/App.tsx:754).

선택한 게임에서 다음 여섯 가지가 하나의 화면 흐름으로 이어져야 한다. 데이터 수집 자동화·계정 OAuth·범용 AI 빌드 생성·전 제작자 확장은 이 핵심을 완결하는 필수 선행 조건이 아니다. 수동 가져오기와 검증된 소수 자료로도 아래 합격을 확인할 수 있다.

1. **입력:** 현재 사용자 스냅샷과 목표 자료를 각각 지정한다. 게임, 대상 패치, HC 여부, Trade/SSF, 캐릭터 정체성, 관측 시각, 언어, 출처가 함께 저장된다. PoB/`.build`만으로 없는 스탯은 0이나 기본값으로 확정하지 않고 미확인으로 남긴다.
2. **비교:** 젬·보조·노드·무기 세트·장비 옵션을 비교하고, 현재와 목표의 출처/역할을 보여 준다. 목표의 장착품 전부를 필수 획득품으로 바꾸지 않는다. 필수·권장·예시·미확인을 분리한다.
3. **다음 단계:** 한 번의 전환에 필요한 레벨/능력치, 소켓·재료, 장비, 자원, 재분배, HC 방어, SSF 획득 조건을 표시한다. 충족·부족·미확인을 구분하며 부족/미확인인 핵심 조건을 AI 설명으로 덮지 않는다. 성공 기준은 “따라야 할 다음 행동과 멈출 이유를 사용자가 알 수 있음”이다.
...
5. **저장·재열기:** 입력·목표·선택 단계·조건 체크·미확인 항목을 저장하고 앱 재시작 후 같은 게임 맥락으로 복구한다. 이후 제작자 목표가 바뀌어도 사용자 현재 상태를 덮어쓰지 않는다.
...
|---|---|---|
| G1 개인 흐름 완결 | 선정한 실제 현재/목표 자료를 UI에서 가져오고, 차이를 확인해 다음 단계 또는 보류 사유를 결정하고, 앱을 닫았다 열어 동일 상태가 복구되는 연속 기록. 중간에 개발자 코드 수정·터미널 명령이 필요하지 않음 | 미확인. POE2 가져오기·저장 연결부터 보완 필요 |
...
남은 것은 실행 담당자의 앱/빌드/테스트 확인, 선택한 개인 빌드의 끝까지 이어지는 사용 확인, 해당 개정 `.build`의 게임 로드 확인, 실제 전환 조건의 적용 검증이다. 검토 책임자가 내릴 제품 결정은 **첫 완결 대상으로 POE2 현재 개인 젬링 자료를 연결할지, POE1 기존 앱 흐름부터 완결할지**다. 어느 쪽이든 전체 게임 범용화나 유료화보다 `입력→현재/목표 비교→다음 단계→저장/재열기`의 증거를 먼저 확보해야 한다.
...
아틀라스는 원래 제품의 부수 장식이 아니다. 전달 문서의 사용자 의도는 **PoB/캐릭터 → 파밍·아틀라스·제작 계획** 중 “어느 노드를 어떤 순서로 찍을지”를 지도 위에서 설명하는 것이다. 이번 자료에는 그 UX의 독립 HTML과 그래프·순서 산출물이 있다. 따라서 앞선 “앱 밖의 연구·가이드 자산”에는 **아틀라스 단계 재생 레퍼런스**도 포함해야 한다. [사용자 목적](D:/Pathcraft-AI/Docs/2026-09-21_ATLAS_VIEWER_REFERENCE_FOR_CODEX.md:9), [로컬 레퍼런스 목록](D:/Pathcraft-AI/Docs/2026-09-21_ATLAS_VIEWER_REFERENCE_FOR_CODEX.md:17).
...
PoB/캐릭터에서 현재 빌드의 부족 자원·목표 단계를 파악한 뒤, 사용자가 파밍 목표(거래 통화 또는 SSF 제작 재료)와 진행 가능한 콘텐츠를 선택하고, **확인된 아틀라스 현재 상태와 가용 포인트**를 별도 입력한다. 그 입력을 검증된 소수 목표 노드/정책에 연결해 다음 단계 순서를 표시하고 근거·조건을 보여 주며 저장·재열기까지 이어야 한다. 아틀라스 결과는 다음 파밍/제작 단계에 필요한 자원의 획득 계획으로 돌아가야 한다. 단순히 범용 HC 전체 순서를 캐릭터 이름 옆에 놓는 것으로 개인화가 완성되지는 않는다. 이 연결은 기존 앱의 파밍·SSF 제작 설명 영역을 확장하는 제안이며, 초기 제품 의도 안에 있다. [목적](D:/Pathcraft-AI/Docs/2026-09-21_ATLAS_VIEWER_REFERENCE_FOR_CODEX.md:9), [현재 파밍/제작 영역](D:/Pathcraft-AI/src/components/sections/FarmingStrategy.tsx:196).
...
**수정된 최종 판단:** 개인용 아틀라스 뷰어의 구체 UX와 입력 데이터 샘플은 이미 있고, R2와 저장 순서의 구조적 근거는 이번에 강화됐다. 따라서 처음부터 뷰어 아이디어를 다시 만들 필요는 없지만, 현재 앱 통합·개인 조건 판정·게임상 유효성·상용 적격성은 별도 미완료다. 기존 보고서의 첫 완결 대상 결정에 아틀라스 축을 포함하되 사용자 현재/목표를 구분하는 핵심 흐름은 유지한다. 이 작업자는 보고서 한 파일만 수정했으며 제품 구현·설치·브라우저 실행·GGPK 추출·권리 확정은 수행하지 않았다.
