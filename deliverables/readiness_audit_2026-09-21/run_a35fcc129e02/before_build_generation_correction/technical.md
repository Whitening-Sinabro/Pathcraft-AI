...
**동작하는 React/Tauri/Python 구현과 별도 연구·플래너 파이프라인이 있다. 그러나 현재 상태를 설치 가능한 완결 앱 또는 입력→가이드→저장→재열기 전체가 검증된 제품으로 판정할 증거는 부족하다.** README의 완료/Mock 표시 대신 실제 명령 등록, 함수 호출, 데이터 읽기와 이번 실행 결과를 기준으로 판단했다.
...
1. **저장 계약 완결:** 링크/구두/.build 입력별 id, game, 스키마/패치 버전, 원천과 저장 성공 여부를 일관되게 보존한다. 빈 상태·이전 빌드 존재·양 게임 전환·API 실패·quota·앱 종료/재시작을 실제 UI에서 검증해야 한다.
2. **에러 계약 완결:** XML/JSON 실패를 올바른 exit/오류로 처리하고 파서→Rust→프런트 경계에서 검증한다. 정상 파싱·깨진 XML·누락 필드·부분 코치 출력에 대해 비용 발생 전에 멈추는지 확인한다.
3. **제품 경로 선택 및 연결:** 현 React 코치, `.build` 생성/여정 DB/HTML, 새 아틀라스 레퍼런스는 각각 가치가 있지만 별도 경로다. 한 대표 사용 시나리오에서 앱 입력→분석→계획/가이드→저장→재열기를 연결하고, 그 경로에 필요한 importer/DB/atlas 계약을 구현해야 한다. 외부 HTML의 존재만으로 앱 기능 완료를 선언하면 안 된다.
