...
# Campaign is a source-attributed adaptation, not a fabricated Kitava tree.
...
for slug,label,sk in [('act1','액트 1 · 철퇴 육성',act1),('act2','액트 2 · 방패벽 전환',act2),('act3','액트 3 · 방패벽 유지',later),('act4','액트 4 · 보상·장비 보완',later),('interlude','막간 · 보상·방패 개선',later)]:
    equip=[{'slot':'Weapon 1','name':'Mace · 철퇴 유형 목표','itemId':None,'levelReq':None,'mods':[],'properties':{},'source':'참조 영상 · 개별 장비 옵션 미확인'}]
    ...
    campaign.append({'id':'campaign-'+slug,'label':label,'creator':'별이슬골짜기 참조 + 탱정 보정; Pathcraft 구성','kind':'campaign_adaptation','ascendancy':'Smith of Kitava (전직 선택 목표)',
         ...
         'skills':sk,'equipment':equip,'passives':[],'weapon_set_nodes':{'1':[],'2':[]},
         'conditions':['각 젬의 실제 사용 요구조건을 충족한 뒤 연결합니다.','탱정의 액트별 Kitava 패시브 스냅샷은 미확인입니다. 타 전직 트리를 자동 적용하지 않습니다.']+(['레벨 7 스킬젬과 방어도 방패를 준비합니다. 스킬젬 등급 7은 캐릭터 레벨 7이 아닙니다.'] if slug=='act2' else [])})
