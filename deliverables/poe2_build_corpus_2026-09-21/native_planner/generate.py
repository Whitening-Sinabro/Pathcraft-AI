...
# Skill visibility was observed by the user in B; this is not an acquisition gate.
SKILL_DISPLAY_INTERVAL = [1, 100]
SKILL_DISPLAY_EVIDENCE = 'B skills visible (msg_db4b49567e07); unchanged original act3 also showed five recommendations (HQ msg_ccc4f724cad0, relay msg_ba97abe53f22); omission is not a proven cause'
# Enabled only after explicit user A2 confirmation relayed by the manager.
PASSIVE_DISPLAY_INTERVAL = [1, 100]
PASSIVE_DISPLAY_EVIDENCE = 'HQ msg_0f2e79de7708 user now sees passives; manager relay msg_ae87d801be97; A2 original weapon_set 0/1/2 + [1,100]'
DISPLAY_NOTE = '\n스킬·보조·패시브의 1~100 범위는 표시 호환용이며 습득·배분·사용 가능 레벨이 아닙니다. 실제 포인트·젬·보조 요구조건을 확인하세요.'
...
def apply_display_compatibility(build):
    build['description'] += DISPLAY_NOTE
    for skill in build['skills']:
        skill['level_interval'] = list(SKILL_DISPLAY_INTERVAL)
        for support in skill.get('support_skills', []):
            support['level_interval'] = list(SKILL_DISPLAY_INTERVAL)
    if PASSIVE_DISPLAY_INTERVAL is not None:
        assert PASSIVE_DISPLAY_EVIDENCE != 'pending_manager_A_result'
        for passive in build['passives']:
            passive['level_interval'] = list(PASSIVE_DISPLAY_INTERVAL)

def validate_campaign_graphs(pack, candidates):
    """Check five final compositions against real source IDs/edges, plus all transition steps."""
    start = candidates['start']['numeric_id']
    assert start == 47175 and 'Warrior' in NODES[start]['classesStart']
    ...
    source_pool = {int(n) for s in ET.parse(CORPUS/'linked_update_pob.xml').getroot().findall('Tree/Spec')[:6] for n in s.get('nodes').split(',')}
    adjacency = {n:set() for n in NODES}
    for n, data in NODES.items():
        for edge in data.get('connections', []):
            v = int(edge['id'])
            if v in adjacency:
                adjacency[n].add(v); adjacency[v].add(n)
    def connected(selected):
        seen, queue = {start}, [start]
        for n in queue:
            for v in adjacency[n] & selected - seen:
                seen.add(v); queue.append(v)
        return seen == selected
    previous, checks = {start}, []
    pack_nodes = {int(n['id']):n for n in pack['tree']['nodes']}
    pack_edges = {tuple(sorted(map(int, edge))) for edge in pack['tree']['edges']}
    assert all(tuple(sorted(edge)) in pack_edges for edge in candidates['selected_source_edges'])
    for stage, candidate in zip(pack['campaign_stages'], candidates['checkpoints']):
        selected = set(candidate['ids_including_start_for_graph'])
        spent = selected - {start}
        assert stage['id'] == 'campaign-'+candidate['id']
        assert all(stage['campaign_checkpoint'][k] == v for k,v in candidate.items())
        assert {int(n['id']) for n in stage['passives']} == spent
        assert [n['stringId'] for n in stage['passives']] == candidate['native_candidate_ids_excluding_start']
        assert len(spent) == candidate['point_budget']['common_spent'] and selected <= source_pool
        assert connected(selected)
        assert stage['weapon_set_nodes'] == {'1':[], '2':[]}
        assert all(not NODES[n].get('ascendancyName') and not NODES[n].get('isAscendancyStart') for n in selected)
        assert all(not NODES[n]['stringId'].startswith('Ascendancy') for n in selected)
        if candidate['id'] != 'act1':
            assert all('two handed' not in ' '.join(NODES[n].get('stats',[])).lower() for n in selected)
        for n in stage['passives']:
            actual = NODES[int(n['id'])]
            assert n['stringId'] == actual['stringId'] and n['stats'] == actual.get('stats',[])
            assert n['connections'] == [str(e['id']) for e in actual.get('connections',[])]
            assert n['x'] == pack_nodes[int(n['id'])]['x'] and n['y'] == pack_nodes[int(n['id'])]['y']
        remaining = set(previous)
        transition = candidate['transition_from_previous']
        for n in transition['refund_numeric_ids_in_safe_order']:
            remaining.remove(n); assert connected(remaining)
        for n in transition['add_numeric_ids_in_safe_order']:
            assert n not in remaining and adjacency[n] & remaining
            remaining.add(n); assert connected(remaining)
        assert remaining == selected
        checks.append({'stage':stage['id'],'ordinary_common_points':len(spent),'source_ids_edges_and_coordinates':True,'connected_both_weapon_sets':True,'transition_every_step_connected':True,'foreign_ascendancy':False,'class_start_exported':False})
        previous = selected
    act2 = candidates['checkpoints'][1]['transition_from_previous']
    assert act2['refund_count'] == len(act2['refund_numeric_ids_in_safe_order']) == 5
    assert act2['add_count'] == len(act2['add_numeric_ids_in_safe_order']) == 13
    bridge = set(candidates['checkpoints'][0]['ids_including_start_for_graph']) - set(act2['refund_numeric_ids_in_safe_order'])
    assert bridge == set(candidates['shield_bridge']['ids_including_start_for_graph'])
    assert connected(bridge) and len(bridge)-1 == 13
    assert all('two handed' not in ' '.join(NODES[n].get('stats',[])).lower() for n in bridge)
    return {'checkpoints':checks,'act2_refunds':5,'act2_additions':13,'shield_bridge_points':13,'shield_bridge_connected':True,'candidate_sha256':sha(CORPUS/'campaign_checkpoint_candidates.json'),'game_verified':False}
...
def generate():
    ...
    for code, filename, title, gate in STAGES:
        ...
        apply_display_compatibility(build)
        ...
        results.append({'code':code,'file':filename,'sha256':sha(OUT/filename),'source':str(source),'source_sha256':sha(source),
                        ...
                        'display_compatibility':{'skills_and_supports':[1,100],'meaning':'Display only; no acquisition or transition level claim','evidence':SKILL_DISPLAY_EVIDENCE,'passive_interval':PASSIVE_DISPLAY_INTERVAL,'passive_evidence':PASSIVE_DISPLAY_EVIDENCE},
                        'limitations':notes})
    latest = results[-1]
    assert latest['passive_count']==151
    assert latest['weapon_set_counts']=={0:104,1:24,2:23}
    ...
    assert pack['display_compatibility']['passives']['production_level_interval'] == PASSIVE_DISPLAY_INTERVAL
    comparisons = []
    ...
    candidates = read(CORPUS/'campaign_checkpoint_candidates.json')
    validation['campaign_graph_checks'] = validate_campaign_graphs(pack, candidates)
    campaign_results = []
    for stage in pack['campaign_stages']:
        candidate = stage['campaign_checkpoint']
        ...
        desc=('별이슬골짜기 캠페인 자료와 탱정 보정에 기반한 Pathcraft 구성. 탱정의 실제 액트 스냅샷이 아닙니다. '
              '저장된 원본의 일반 패시브만 공통 배분으로 구성했습니다. 워브링어/타이탄 전직은 가져오지 않았습니다. 배열은 저자의 클릭 순서가 아닙니다.\n'
        ...
        passives=[]
        for n in stage['passives']:
            note='Pathcraft 공통 배분 · 일반 '+str(candidate['point_budget']['common_spent'])+'점 조건. 레벨·저자 클릭 순서가 아닙니다.'
            if n.get('attribute_choice'): note+=' 실제 장비·젬 요구치를 비교하여 능력치를 선택하세요.'
            if n['name'] in candidates['notable_effect_review']: note+=' '+candidates['notable_effect_review'][n['name']]
            passives.append({'id':n['stringId'],'weapon_set':0,'additional_text':note})
        build={'name':'캠페인 참조 '+stage['label'],'author':'별이슬골짜기 가이드 / 탱정 보정 / Pathcraft 정리',
               'link':stage['source']['url'],'description':desc,'passives':passives,'skills':entries,'inventory_slots':equipment}
        apply_display_compatibility(build)
        check_shape(build)
        ...
        campaign_results.append({'file':filename,'sha256':sha(OUT/filename),'common_pack_stage':stage['id'],
                                 'all_ids_checked':True,'official_key_type_check':True,'ascendancy_omitted':True,
                                 'passive_count':len(passives),'all_passives_common':True,'class_start_exported':False,'point_budget':candidate['point_budget'],
                                 'native_ids_exactly_match_accepted_candidate':[p['id'] for p in passives]==candidate['native_candidate_ids_excluding_start'],
                                 'display_compatibility':{'skills_and_supports':[1,100],'evidence':SKILL_DISPLAY_EVIDENCE,'passive_interval':PASSIVE_DISPLAY_INTERVAL,'passive_evidence':PASSIVE_DISPLAY_EVIDENCE},
                                 ...
                                 'limitation':'Accepted Pathcraft common-node composition, not author act restoration; no inferred acquisition levels, ascendancy, DPS, HC or new-campaign game PASS.'})
    ...
    assert baseline_path.exists(), 'Historical baseline must remain available'
    if baseline_path.exists():
        ...
        prechange=OUT/'versions/pre_campaign_composition_20260921'
        assert sha(baseline_path)==sha(prechange/'ux_structure_baseline.json'), 'Never overwrite historical baseline'
        for filename in [r['file'] for r in results]:
            before, after = read(prechange/filename), read(OUT/filename)
            assert after['description']==before['description']+DISPLAY_NOTE
            after['description']=before['description']
            for skill in after['skills']:
                skill.pop('level_interval')
                for support in skill.get('support_skills',[]): support.pop('level_interval')
            if PASSIVE_DISPLAY_INTERVAL is not None:
                for passive in after['passives']: passive.pop('level_interval')
            assert before==after, 'Snapshot data and existing annotations must remain unchanged'
        current={f.name:structure(read(f)) for f in OUT.glob('*.build')}
        assert set(current)==set(baseline) and len(current)==10
        normalized={}
        for filename, build in current.items():
            # Only specified display ranges and accepted campaign nodes may differ structurally.
            for skill in build['skills']:
                assert skill.pop('level_interval') == [1,100]
                for support in skill.get('support_skills',[]):
                    assert support.pop('level_interval') == [1,100]
            for passive in build['passives']:
                if PASSIVE_DISPLAY_INTERVAL is None: assert 'level_interval' not in passive
                else: assert passive.pop('level_interval') == PASSIVE_DISPLAY_INTERVAL
            if filename.startswith('campaign-'):
                candidate = next(c for c in candidates['checkpoints'] if filename=='campaign-'+c['id']+'.build')
                assert build['passives'] == [{'id':n,'weapon_set':0} for n in candidate['native_candidate_ids_excluding_start']]
                assert baseline[filename]['passives']==[]
                build['passives']=[]
            normalized[filename]=build
        assert normalized==baseline, 'Unexpected structural change outside accepted nodes/display ranges'
        ...
        validation['ux_correction']={'status':'passed','structural_baseline_sha256':sha(baseline_path),
                                     'historical_baseline_preserved':True,'allowed_diff_checks_passed':True,
                                     'allowed_structural_changes':['skills/supports level_interval [1,100] display compatibility','exact accepted campaign common-node IDs, excluding free start']+(['manager-approved passive display ranges'] if PASSIVE_DISPLAY_INTERVAL is not None else []),
                                     'five_snapshot_ids_and_all_other_structure_unchanged':True,'annotations':'description/additional_text remain prose fields; campaign conditions and source effect caveats explicitly generated','debug_text_removed':True,
...
if __name__=='__main__':
    import sys
    if '--lund-authored' in sys.argv:
        sys.path.insert(0,str(CORPUS))
        from lundburgerr_authored.compose import native_outputs
        native_outputs(read(CORPUS/'planner_data/build_data.json'))
    elif (CORPUS/'lundburgerr_authored/authored_document.json').exists():
        raise SystemExit('Use --lund-authored for authorized campaign replacement; latest/archive remain frozen')
    else:
        generate()
