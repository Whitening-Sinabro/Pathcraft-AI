...
(async () => {
  ...
  try {
    ...
    await page.goto(pathToFileURL(path.join(__dirname, '40분대_구매와제작_추가확인.html')).href);
    ...
    await page.waitForFunction(() => Math.abs(document.querySelector('video').currentTime - 210) < .1);
