...
TITLE = '05_06_설명장면_재확인'
scenes = json.loads((HERE / 'replay_scenes.json').read_text(encoding='utf-8'))
...
<title>Skadoosh · 05·06 설명 장면 재확인</title>
<style>
...
coverage = json.loads((HERE / 'inspection_coverage.json').read_text(encoding='utf-8'))
manifest = {'created_at_utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
            ...
            'inspection_coverage': coverage, 'source_alignment_file': 'source_alignment.json',
            'replay_seconds': sum(x['duration'] for x in scenes),
...
print(json.dumps({'html': str(HERE / (TITLE + '.html')), 'clips': len(clips),
                  'replay_seconds': manifest['replay_seconds'], 'inspected_frames': coverage['total_frames_reviewed']}, ensure_ascii=False))
