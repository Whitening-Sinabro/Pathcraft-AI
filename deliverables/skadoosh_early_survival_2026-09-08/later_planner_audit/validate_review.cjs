...
(async () => {
  ...
  try {
    ...
    await page.goto(pathToFileURL(path.join(__dirname, '후속_플래너_정밀확인.html')).href);
    const results = [];
    for (const id of ["cc_swap", "cc_eff", "cc_clear", "awt_remove", "awt_gems", "awt_sets", "awt_demo", "paquate_swap", "seismic_finish", "paquate_rare", "late_mace", "wooden_wall"]) {
