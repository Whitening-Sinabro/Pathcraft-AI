...
LINE_FIXES: list[tuple[str, str | None]] = [
    ...
    ("지진 함성 → 우주의 영사 + 효율 II + 격노하는 함성",
     "지진 함성 → 우주의 영사 + 효율 II + 격노하는 함성 + 기동성 + Magnified Area II"),
    ("지진 함성 → 우주의 영사 + 효율 II + 격노하는 함성 + 기동성",
     "지진 함성 → 우주의 영사 + 효율 II + 격노하는 함성 + 기동성 + Magnified Area II"),
    # 89레벨에 붙인 범위 보조. 이유는 본인 발언(9/12 방송 01:22:01) —
    # 함성 피해 증가는 폭발에 안 먹고, 범위가 시체와 가시를 더 많이 맞혀서 낫다.
...
# 여러 줄짜리 추천 옵션 블록은 줄 단위로 못 바꾼다. 통째로 치환하되,
# 새 블록이 이미 있으면 건너뛰어 재실행에 안전하게 한다.
# 근거: 9/12 방송 화면 실측(00:56:15 투구 툴팁) + 89레벨 공개 스냅샷 장비.
BLOCK_FIXES: list[tuple[str, str]] = [
    ("1. 최대 생명력\n2. 현재 부족한 화염·냉기·번개 저항\n3. 초당 생명력 재생\n4. 방어도",
     "1. 함성 피해 증가 (T1 은 50~75%)\n"
     "2. 함성 사용 시 최대 생명력의 4~5% 회복 (T1). 함성의 생명력 비용을 이걸로 갚는다\n"
     "3. 최대 생명력\n"
     "4. 방어도의 일부가 원소 피해에도 적용\n"
     "5. 현재 부족한 화염·냉기·번개 저항\n"
     "제작자 89레벨 투구 실측(9/12 방송 00:56:15 화면): 방어도 67% · 생명력 +139 ·\n"
     "함성 피해 59% · 함성 시 생명력 5% 회복 · 방어도 37% 원소 적용 · 냉기 저항 36%.\n"
     "아이템 레벨 80 · 퀄리티 20% · 방어도 749 · 완성 후 타락. 본인 제작이다"),
    ("1. 모든 근접 스킬 레벨 +\n2. 최대 생명력\n3. 현재 부족한 화염·냉기·번개 저항\n"
     "4. 초당 생명력 재생\n5. 방어도\n6. 공격 속도",
     "1. 모든 근접 스킬 레벨 +\n2. 방어도 증가\n3. 최대 생명력\n"
     "4. 현재 부족한 화염·냉기·번개 저항\n5. 최대 마나\n"
     "제작자 89레벨 장갑 실측: 방어도 74% · 생명력 +140 · 마나 +56 · 근접 스킬 +2 ·\n"
     "냉기 30% · 번개 18%. 직전 장갑의 '공격에 화염 피해 추가'는 이 빌드의 축이 아니라 버렸다"),
]
