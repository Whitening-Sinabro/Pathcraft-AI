...
| scripts/probe_gemling_jewels.py | 화염파 젬링 주얼 등급별(중/상/최상) 거래소 검색 + 시장 천장 훑기(`--top-sweep`). 등급 기준은 실캐릭 Lv97 착용 8개의 실측 분포, 스탯 id 는 공식 trade2 인덱스와 대조한 것만. `?q=` 무상태 링크가 정본(검색 id 는 서버별·만료) | 아니오 |
