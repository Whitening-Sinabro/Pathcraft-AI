...
HC 여정 DB(`python/hc_journey/`) #1 규칙 자동 초안 완료 + F 자막 재추출 + 판독 신호원 + 후보 58건 판정(채택 21·기각 24·보류 13 → `rule_decisions.json`) → 규칙 21건·규칙 노트 34건. 규칙 텍스트 적대검증 2회(오독 2건 수정).

**다음**: ① 추적기가 잡는 변화마다 플래너·사이드카 갱신(15분 주기, scratchpad `sb_track/`) — 갱신되면 `make_hc_gemling_spec.py` → 빌드를 다시 돌려야 필터에 반영된다 ② 채굴본 293건을 사용자용 액트별 시트로 압축(브라우저 페이지 제안해 둠) ③ 커밋 안 함 — 워킹트리에 이번 작업분이 그대로 있다(HC 여정 DB 파일만 커밋됨) ④ 여정 DB 보류 13건 해제 조건: 거래 지식용 trade 트리거 설계, 유탄이 '위에서 떨어뜨리는 스킬'인지 GGPK 확인, 보조 젬 선택 트리거. 그다음 새 크리에이터 소스(`CREATOR_SOURCES`) 추가
