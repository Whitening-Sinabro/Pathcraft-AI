...
**다음**: ① **소싱 #2 허락 대기** — `.claude/status/hc_creator_sourcing.md` 의 허락 목록 A(Blazeworks: Mobalytics 탭 export + ninja `BlazeW-3944` 1회 + 자막) · B(MisoxShiru: Mobalytics 탭 export + 자막) · C(디넬: ninja `DNEL-7382` 캐릭터 조회 + 폴링 + 자막 2) · D(선택, 임성빈 Mobalytics 프로필 열람). 허락받은 항목부터 픽스처 → `CREATORS` → 파이프라인 ② 탱정 0.5.5 실캐릭 4밴드(로컬 xml)는 "같은 사람 두 번째 빌드" 항목 병합 결정 뒤 ③ 추적기가 잡는 변화마다 플래너·사이드카 갱신(15분 주기, scratchpad `sb_track/`) — 갱신되면 `make_hc_gemling_spec.py` → 빌드 재실행 ④ 채굴본 293건 액트별 시트 압축 ⑤ 여정 뷰 사용자 피드백. 여정 DB SSF 규칙은 SSF 소스가 생기면 `league/ssf` 로, 새 크리에이터 소스(`CREATOR_SOURCES`) 추가
