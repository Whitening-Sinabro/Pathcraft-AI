...
- **감사 단계 (Audit only)**:
  - 감사 도메인이 독립적이고 서로 영향 없으면 한 세션에 2개 이상 병합 가능 (예: F2+F7은 데이터 소스 겹침 없음)
  - 단, fix 실행 단계에서는 "1~2개씩 구현→확인" 복귀 (수정은 회귀 리스크 있음)
- **수정 분류 (Fix scope)**:
  - **"수정" = 코드/데이터 로직 변경** (기존 동작 바꾸는 변경). 별도 task 분리 대상
  - **"감사 단계 허용" = 테스트 추가 + 주석/`_meta` 보강** (기존 로직 불변). 감사 중 즉시 가능
