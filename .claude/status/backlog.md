...
- [ ] **F3a-fix-1 (🔴)** — `farming_mechanics.json` + `farming_strategies.json` + `farming_strategy_system.py` (1450 lines) ORPHAN 삭제 or `_archive/` (15min)
- [ ] **F3a-fix-3 (🟡)** — `Inscribed Ultimatum` 주석에 Cobalt 버전 추가 (5min)
- [ ] **F3b-fix-1 (🟡)** — `_HEIST_OBJECTIVES` 출처 주석 (10min)
- [ ] **F3b-fix-2 (🟡)** — `_HEIST_HANDPICKED_AREAS` 선정 근거 주석 (10min)
- [ ] **F4-fix-1 (🔴)** — `sanavi_tier_data.json` + `sanavi_tier_parser.py` ORPHAN 삭제 or `_archive/` (10min)
- [ ] **F5-fix-1 (🟡)** — `syndicate_layouts.json` `_meta.source` 추가 (15min)
- [ ] **Phase F legacy cleanup 묶음**: mod_pool + sanavi 2 + farming 3 = 6 파일 일괄 정리 (권장, 30min)
