...
- **이미지 삽입 메뉴는 한 번에 안 열릴 때가 있다.** 16장 넣는 문서에서 3번째 이미지에서
  터졌다. `Escape` → 메뉴 재클릭 → hover 를 4회까지 재시도하도록 8개 주입기 전부 고쳤다.
  실패하면 문서가 반쯤 채워진 채로 남으므로 그대로 다시 돌리면 된다(첫 단계가 비우기다).
- 주입기는 **돌리기 전에 구문 검사**한다. 오타 하나로 브라우저 왕복을 통째로 날린다.
  `node -e "new Function('page','('+require('fs').readFileSync(f,'utf8')+')')"`.

## 디스코드 첨부를 '교체'할 때

`update_poe2_cards.mjs` 는 **같은 파일명이 이미 붙어 있으면 업로드를 건너뛴다**(중복 방지).
파일명은 그대로고 내용만 바뀐 zip 을 갈아 끼울 때는 그 가드가 정확히 반대로 동작한다.
교체는 옛 첨부를 `keep` 목록에서 빼고 새로 올려야 한다 —
`scratchpad/replace_card_zip.mjs` 가 그 형태다(제목 한 줄 대조 후에만 PATCH, 429 재시도 포함).

**본문만 보고 "고쳤다"고 하지 말 것.** 첨부 zip 은 따로 내려받아 안의 파일명을 확인한다.
본문이 맞는데 zip 이 옛것인 상태가 성립한다.


## 발행 파이프라인에서 또 밟은 것 (2026-09-04)

- **`v4_*.json` 의 `tabs` 는 장을 추가할 때 같이 고쳐야 한다.** 점화 하코 편에 제5장을
  새로 만들고 첨부를 제6장으로 밀었더니, 설정에 제6장이 없어서 **발행본에서 통째로
  사라질 뻔했다**. 빌드 로그의 `sections:` 줄과 조각별 bytes 를 눈으로 대조할 것.
- **이미지 마커는 `<p>` 로 감싸져야 주입기가 조각을 가른다.** 생성기는 소스의 맨몸
  `@@IMG:name@@` 을 그대로 내보내는데, 주입기 정규식은 `<p[^>]*>@@IMG:(\w+)@@</p>` 다.
  1차 주입이 `조각 1 · 이미지 0` 으로 끝나 스크린샷 3장이 통째로 빠졌다. 페이로드를
  만든 뒤 **`조각 N · 이미지 M` 이 기대값인지 로그로 확인**하고 넣을 것.
- **구글 닥 본문은 `document.body.innerText` 로 검증할 수 없다.** 캔버스 렌더라
  사이드바 개요만 잡히고 본문은 안 잡혀서 전 항목이 false 로 나온다(판정 불가지
  실패가 아니다). 검증은 **`/document/d/<ID>/export?format=txt` 를 같은 오리진에서
  fetch** 해서 한다 — 신규 문구 존재와 **옛 문구 부재**를 같이 본다.
- **빌드·주입 도구가 옛 세션 스크래치패드에 산다**(`build_gdoc_v4.mjs`, `v4_*.json`).
  세션 폴더가 지워지면 발행 경로가 끊긴다. 옮길 것 — 아직 안 옮겼다.

## 하드코어 문서는 차이분이 아니라 전체 가이드로 쓴다 (2026-09-04 방향 전환)

점화 하코 편을 "운용법은 본 가이드를 보라"는 **차이분 문서**로 쓴 게 틀렸다.
사용자 지적: **"소코는 엄청 긴데 하코는 짧아서 확인이 불가능하고, 이걸 보고 가이드도
못 한다."** 독자는 문서 두 개를 왕복하지 않는다.

- **하드코어 편은 1레벨부터 엔드게임까지 혼자 따라갈 수 있어야 한다.** 젬 링크 ·
  액트별 장비 접두접미 · 퀘스트 보상 · 어센던시 순서까지 전부 싣는다.
- **뼈대는 소프트코어 PoB 가 아니라 하드코어에서 검증된 세팅을 쓴다.** 점화 편은
  아르세리나(SC) 대신 **임성빈의 하드코어 빌드**를 뼈대로 삼고 SC 는 원리 설명에만 썼다.
- 결과: 본문 5,779 -> 21,556자, 장 8 -> 12, 표 178행.

## Mobalytics 빌드 문서는 자막보다 훨씬 낫다 (브라우저 필수)

봇에 403 이라 WebFetch 로는 안 되고 **Playwright 로 열어야** 한다. 열면 자막에 없는
것이 통째로 나온다 — **액트별 탭마다** 젬·보조젬·장비 접두접미·룬·플라스크·퀘스트
보상·트리 포인트, 그리고 제작자가 쓴 주석.

- 탭은 섹션마다 따로 있다. `innerText` 가 탭 이름과 정확히 같고 높이 60px 미만인
  요소를 **전부 클릭**해야 Build Variants·Equipment·Skill Gems·Passive Tree 가 같이 넘어간다.
- 크리에이터의 **영상 설명란**을 먼저 볼 것. `yt-dlp --print "%(description)s"` 로 받는다.
  ds lily 는 설명란에 **"6:12 에서 나는 Tornado 를 말한 것"** 이라고 자기 오류를 정정해
  놨고, Mobalytics 링크와 다른 크리에이터 2명 링크도 거기 있었다.

## 크리에이터를 검증자로 쓸 때 무게를 확인할 것

같은 빌드 가이드를 냈다고 그 사람이 **이번 리그에 그걸 하는 것은 아니다**.
ds lily 는 화염파 젬링 가이드를 냈지만 0.5.5 개인 계획은 **기름 연사 무술가**이고
"가이드는 아마 안 만들 것" 이라고 했다. 그녀의 중재자 처치도 **"몇 달 전"**(이전 패치)다.
문서에 "검증했다" 로만 적으면 없는 지지를 만들어 낸다 — **본인 계획 영상까지 확인**하고
무게를 따로 적는다.

## 크리에이터 주장 대 게임 데이터가 어긋나면 데이터가 이긴다

ds lily 가 문서와 영상 양쪽에 **"0.5.5 의 새 Xoph's Blood 가 Avatar of Fire 를 준다"**
고 적었다. poe2db 확인 결과 그 옵션이 없고 **최종 변경이 0.4.0** 이다. 그 전제 위에
"고정 화염 피해를 신경 안 써도 된다" 는 조언이 붙어 있었는데, 전제가 틀려 조언도 틀린다.
**패치노트에 없다"만으로 결론내지 말고 아이템 페이지를 직접 볼 것** — 0.5.5 는 밸런스
섹션 자체가 없어서 "패치노트에 없다" 가 아무 증거도 못 된다.

## 플래너를 설치할 때 옛 이름이 남는다 (2026-09-04)

`build_planner/*.build` 를 게임 폴더에 복사할 때, **파일명이 바뀐 세대의 옛 파일이 그대로
남아 인게임 목록에 중복으로 뜬다.** 점화 편에서 실제로 그랬다 —
`Ignite 1 Lv1-5`(옛) 와 `Ignite 1 Lv1-14`(신), `Ignite 2 Act Tactician`(옛) 와
`Ignite 2 Lv15-51 Act`(신). **옛 쪽 레벨 표기가 틀린 채로 목록에 보인다.**

지우기 전에 **JSON 으로 파싱해 `name` 필드만 다른지 확인**한다. 점화 편은 패시브
18/69 · 스킬 4/8 · 슬롯 2/10 · `level_interval` 이 전부 같고 `name` 만 달라 안전하게
지울 수 있었다. 바이트 해시로 비교하면 `name` 때문에 항상 "다름" 이 나와 판단이 안 된다.

게임 폴더는 git 이 아니라 **복구가 안 된다.** 내용 동일성을 확인하기 전에는 지우지 말 것.
